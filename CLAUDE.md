...
- `scripts/check_sync.sh` -- compares local vs public RPC block heights (Substrate JSON-RPC). Exit codes: 0=synced, 1=syncing, 2=diverged, 3=local_rpc_error, 4=public_rpc_error, 5=config_error, 6=tool_dep_error, 7=container_error.
...
- **NETWORK variable:** Only sets metrics labels. Chain spec is hardcoded to finney in `bittensor.yml`. To run testnet, override `--chain` and `--bootnodes` via `EXTRA_FLAGS`.
